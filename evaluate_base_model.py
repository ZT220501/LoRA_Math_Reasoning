...
def evaluate_base_model(model, tokenizer, test_data, sampling_budget=1, max_new_tokens=512, device="cuda", base_model=False, temperature=1.0, top_p=0.95, top_k=0):
...
if __name__ == "__main__":
    ...
    eval_save_dir = f"eval_results_gsm8k/{model_name}_base_model_sampling_budget_{sampling_budget}"
    os.makedirs(eval_save_dir, exist_ok=True)
    base_acc, base_correct, total, base_result_json = evaluate_base_model(
        model=base_model, 
        tokenizer=tokenizer, 
        test_data=test_data, 
        sampling_budget=sampling_budget, 
    ...
    # Save the base model result to a json file
    with open(os.path.join(eval_save_dir, "base_model_result.json"), "w") as f:
        json.dump(base_result_json, f)
    print(f"\nBase Model Accuracy: {base_acc:.2f}% ({base_correct}/{total})")